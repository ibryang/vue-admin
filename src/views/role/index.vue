      ...
      <el-card class="box-card">
        ...
        >
          ...
          >
            ...
            <el-form-item label="权限字符" prop="role_key">
              <el-input
                v-model="form.role.role_key"
                placeholder="请输入权限字符"
              />
            </el-form-item>
            <el-form-item label="角色顺序" prop="role_sort">
              <el-input-number
                v-model="form.role.role_sort"
                controls-position="right"
                :min="0"
              />
            </el-form-item>
            <el-form-item label="状态">
              <el-radio v-model="form.role.status" label="2">正常</el-radio>
              <el-radio v-model="form.role.status" label="1">锁定</el-radio>
            </el-form-item>
        ...
        >
          <el-form :model="form" label-width="80px">
            <el-form-item label="角色名称">
              <el-input v-model="form.role.role_name" :disabled="true" />
            </el-form-item>
            <el-form-item label="权限字符">
              <el-input v-model="form.role.role_key" :disabled="true" />
            </el-form-item>
            <el-form-item label="权限范围">
              <el-select v-model="form.role.data_scope">
                <el-option
            ...
            <el-form-item v-show="form.role.data_scope == '2'" label="数据权限">
              <el-tree
                ref="dept"
                :data="deptOptions"
                show-checkbox
  ...
  data() {
    return {
      ...
      rules: {
        ...
        role_sort: [
          { required: false, message: "角色顺序不能为空", trigger: "blur" },
        ],
      },
    };
  },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$confirm(
        ...
        .then(function () {
          changeRoleStatus({ role_id: row.role_id, status: row.status }).then(
            (res) => {
              _this.getList()
    ...
    // 获取下级节点不为空的id
    get_children_menu_id(data, list = []) {
      data.forEach((item, index) => {
        if (item.children) {
          list.push(item.id);
          if (item.children.length !== 0) {
            return this.get_children_menu_id(item.children, list);
          }
        }
      });
      return list;
    },
    getTreeIds(data_ids, data_list) {
      var child_ids = this.get_children_menu_id(data_list);
      var id_list = data_ids.filter(function (v) {
        return child_ids.indexOf(v) == -1;
      });
      return id_list;
    },
    /** 新增按钮操作 */
    ...
    handleUpdate(row) {
      this.reset();
      const role_id = row.role_id || this.ids;
      this.getMenuTreeselect();
      getRole(role_id).then((response) => {
        ...
        this.title = "修改角色";
        this.isEdit = true;
        let menu_ids = this.getTreeIds(response.menus, this.menuOptions);
        this.$nextTick(() => {
          this.$refs.menu.setCheckedKeys(menu_ids);
        });
      });
    },
    /** 分配数据权限操作 */
    handleDataScope(row) {
      ...
      getRole(row.role_id).then((response) => {
        ...
        this.title = "分配数据权限";
        let dept_ids = this.getTreeIds(response.depts, this.deptOptions);
        this.$nextTick(() => {
          this.$refs.dept.setCheckedKeys(dept_ids);
        });
      });
    },
    /** 提交按钮 */
    submitForm: function () {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.role.role_id !== undefined) {
    ...
    submitDataScope: function () {
      if (this.form.role.role_id !== undefined) {
        this.form.dept_ids = this.getDeptAllCheckedKeys();
    ...
    handleDelete(row) {
      let _this = this
      const role_ids = row.role_id || this.ids;
      this.$confirm(
        '是否确认删除角色编号为"' + role_ids + '"的数据项?',
        "警告",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
